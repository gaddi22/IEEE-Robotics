...
void runPath(double angle, double distance){
  ...
  curAngle = curAngle + (trudangle - angle); // find error
                                                    // might want this as its own funtion.
  double x = currentCoord[0];
  double y = currentCoord[1];
  Serial.print("angle to convert: ");
  Serial.println(angle);
  double rad      = degToRad(trudangle);            //angle in radians
  Serial.print("rad: ");
  Serial.println(rad);
  Serial.print("change x: ");
  Serial.println(sin(rad) * trudist/304.8);
  Serial.print("change y: ");
  Serial.println(cos(rad) * trudist/304.8);
  currentCoord[0] = x + sin(rad) * trudist/304.8;
  currentCoord[1] = y + cos(rad) * trudist/304.8;                 //new location
